...
$(function() {
	...
	myDropzone.on("success", function(file, response) {
		...
		if (response.status == 'failed') {
			...
			$('#division_image').val(response.image);
			$('#division_active_photo').attr('src', asset_url + response.image);
	...
	$('#image_sizes, #thumbnails').on("click", ".btn-image", function() {
		...
		$('#division_image').val(image);
		$('#division_active_photo').attr('src', asset_url + image);
